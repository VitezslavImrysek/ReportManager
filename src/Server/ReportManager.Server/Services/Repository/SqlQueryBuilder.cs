...
namespace ReportManager.Server.Services.Repository
{
	public sealed class SqlQueryBuilder
	{
			...
			int? pageSize)
        {
            ...
            var realCols = (selectedColumns ?? []).Where(c => cols.ContainsKey(c)).ToList();
            if (realCols.Count == 0)
            ...
            // WHERE
            var parameters = BuildWhere(query, cols, sb);

            // ORDER BY
            BuildOrderBy(query, cols, sb);

            // PAGING
            BuildPaging(pageIndex, pageSize, sb);
        ...
        private static List<SqlParameter> BuildWhere(QuerySpecDto query, Dictionary<string, ColumnInfo> cols, StringBuilder sb)
        {
        ...
        private static void BuildOrderBy(QuerySpecDto query, Dictionary<string, ColumnInfo> cols, StringBuilder sb)
        {
            var order = new List<string>();
            foreach (var s in query.Sorting ?? [])
            {
                if (!cols.TryGetValue(s.ColumnKey ?? string.Empty, out var col) || !col.SortEnabled)
                    continue;
        ...
        private static void BuildPaging(int pageIndex, int? pageSize, StringBuilder sb)
        {
            if (pageSize != null)
            {
